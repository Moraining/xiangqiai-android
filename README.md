...
- **本 App 的解法**：不依赖 COOP，直接把皮卡鱼 C++ 用 NDK 交叉编译成**原生 arm64 可执行文件**
  （`make build COMP=ndk ARCH=armv8-dotprod`），App 内起子进程跑它。
  原生引擎用 pthread 自动吃满手机全部 CPU 核心，是**真多线程 + SIMD**
- 网页（原站 UI）与原生引擎之间通过一层轻量桥通信：JS → `addJavascriptInterface` →
  引擎 stdin；引擎 stdout → Java 线程 → `evaluateJavascript` 推回页面。界面与原站完全一致
...
2. 把 `assets/native/pikafish-arm64`、`assets/native/pikafish.nnue` 放入
   `app/src/main/assets/native/`（可从 GitHub Actions 构建产物中获取，或自行
   `cd <pikafish>/src && make build COMP=ndk ARCH=armv8-dotprod` 编译）
...
└── app/
    ...
    └── src/main/
        ├── AndroidManifest.xml
        ├── java/com/xiangqiai/app/MainActivity.java   # 服务器 + WebView + 原生引擎桥
        ├── res/                                       # 布局 / 图标 / 主题
        └── assets/
            ├── native/                                # 原生皮卡鱼引擎（Actions 编译生成）
            │   ├── pikafish-arm64                     # arm64 可执行文件（多线程）
