...
4. 等 **5~10 分钟**（要下载 NDK、编译原生引擎、构建 APK），变**绿色** = 构建成功
...
3. 解压得到 **app-debug.apk**（debug 版已自动签名，可直接安装）
...
| APK 体积较大（几十 MB） | 正常：包含原生引擎 + 神经网络文件 |
| 安装后「设置 → 浏览器环境」显示不支持多线程 | 说明这台设备不是 arm64 或原生引擎启动失败，已自动回退 wasm 单线程，可正常使用；多线程需 arm64 手机（2017 年后绝大多数机型） |
